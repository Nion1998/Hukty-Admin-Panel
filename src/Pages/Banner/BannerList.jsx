...
const BannerList = () => {
  const { deleteBanner, changeActiveStatusBanner } = useContext(AuthContext);
  ...
  const changeActiveStatus = (id, is_active) => {
    ...
    changeActiveStatusBanner(id, is_active)
      .then((response) => {
  ...
  // search filter
  const filteredCategories = bannerData.data.results.filter((data) =>
    ...
    <div className=" container-fluid px-lg-3 ">
      ...
      <div className="mb-2  d-lg-flex justify-content-between align-items-center">
        <div className="">
          ...
          <span className="path-text-span">Banner List</span>
          <div className="page-title">Banner List</div>
        </div>
      </div>
      {/* Table div */}
